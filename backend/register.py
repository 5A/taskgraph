...
"""register.py:
This script helps you to register a new user into config.
"""
...
# this package
from server_config import dump_config_to_file, load_config_from_file, UserAccessLevel, UserConfig
...
if __name__ == "__main__":
    print("Registering new user into server config, Ctrl-C to cancel.")
    print("========================================================")
    ...
    possible_access_levels: str = ", ".join([str(e.value) for e in UserAccessLevel])
    print("Please specify the access level of the new user, possible values are {}".format(
        possible_access_levels))
    print("These values corresponds to {}".format([e for e in UserAccessLevel]))
    ...
    if ans == 'y' or ans == '':
        ...
        print("Check if the new config is correct and rename it to server.config.json if everything is OK.")
    else:
        print("Aborted.")
